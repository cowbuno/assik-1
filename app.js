...
async function main() {
  ...
  while (true) {
    ...
    if (action === "1") {
      ...
      const encryptedData = encrypt(data); // Encrypting
      const dataToSign = JSON.stringify(encryptedData); // Converting encrypted to a string
      ...
      if (latestBlock) {
        ...
        console.log(
          "Decrypted Data of the latest block:",
          decrypt(latestBlock.data)
        );
